...
현장에서는 스톱워치·랩 기록·진동 알림 등으로 변형했다.
...
- 안 쓰는 기능 파일은 빌드에서 빼는 게 좋다
- 양산 앱이 반려당한 이유가 “똑같은 앱”이었으므로, **기능 조합을 다르게** 가져가는 것도 전략이 될 수 있다 (법·스토어 정책은 별도 확인)
...
**셸 + 링크 규칙 + 옵션 네이티브 화면**이 합쳐져야 현장에서 쓸 만한 템플릿이 된다.

예제는 전부 `examples/` 아래에 있고, 회사 코드는 포함하지 않았다.
